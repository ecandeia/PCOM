...
%*******************
\begin{figure}[h]
\centerline{\psfig{figure=./Figuras/sistemasimples.eps,width=9cm}}
...
ou seja
\begin{eqnarray}
\mid R(\omega)\mid &=& \mid S(\omega)\mid\mid H(\omega)\mid \nonumber \\
\theta_R(\omega) &=& \theta_S(\omega) + \theta_H(\omega) \nonumber
\end{eqnarray}
...
Deve fazer a ressalva de que há diferença entre o atraso ($t_d$) constante e a fase ($\omega t_d$) constante. Nesse último caso, se a fase for constante, haverá distorção, e pode-ser perceber que, se a fase for proporcional à frequência, ou seja $arg(H(\omega) = -\omega t_d$, então
\[
...
Outro parâmetro derivado da relação entre fase e atraso, é o {\bf atrso de grupo}, definido como
\[
\tau_d = -\frac{d arg(H(\omega)}{d\omega}
\]
e mede a variação da fase. 

%\vspace{3.5cm}
%\begin{figure}[htb]
%	\centering
%		\scalebox{0.4}{\includegraphics[10cm,0.5cm][0.5cm,0.5cm]{Figuras/homegalinear.jpg}}
...
envelhecimento, etc;
\item filtros de mais alta ordem são facilmente realizáveis;
\item podem ser facilmente modificados, alterando-se apenas o algoritmo.
\end{itemize}
...
Nos sistemas de multiplexação em frequência como o FDM ({\em Frequency
  Division Multiplexing}), o sinal é distorcido  mas não afeta os canais vizinhos.
...
atenuados de $k/2$, deslocados de $\pm T$ e atrasados de $t_d$, como mostrado na Figura~\ref{fig:pulsosatrasados}.
...
Um exemplo simples de equalizador são os filtros transversais, como estão mostrados na Figura~\ref{fig:filtrotransversal}. Nesse filtro o sinal recebido $s(t)$ passa por uma linha que o atrasa e em seguida multiplica por um coeficiente $c_i$, dessa forma
\[
...
%*******************
\begin{figure}[h]
\centerline{\psfig{figure=./Figuras/filtrotransversal.eps,width=8cm}}
\caption{\label{fig:filtrotransversal}Filtro transversal usado como equalizador.}
\end{figure}
%*******************

\begin{exemplo} Sendo $x(t)$ a entrada de um sistema linear e $y(t) = k_1 x(t-t_1) + k_2 x(t-t_2)$ sua saída, obtenha a expressão do equalizador baseado em linhas de atraso para ser colocado na saída do sistema para que se obtenha uma transmissão sem distorção.
...
\begin{eqnarray}
H(\omega) &=& k_1e^{-j\omega t_1} + k_2 e^{-j\omega t_2} \nonumber \\
&=& k_1e^{j\omega t_1}(1 k e^{-j\omega t_0}) \nonumber
\end{eqnarray}
...
\begin{exemplo} Um sinal $m(t)cos\omega_c(t)$ é transmitido através um canal com a
...
Pode-se observar na Figura~\ref{fig:homegamultipercurso} que a maginitude e a fase de $H(\omega)$ tem período $2\pi/\Delta t$.
...
A função $\mid G(\omega)\mid$ é chamada de {\bf Densidade espectral de energia} (ESD).
...
%
%Portanto para uma dada aplicação podemos estabelecer um critério de erro no qual consideramos o sinal 
%limitado em $B Hz$. Por exemplo, podemos usar o critério de que um sinal é limitado em $B Hz$ se a 
%contribuição das componentes espectrais acima de $B Hz$ for menor que $x$ por cento da energia do sinal.
...
\[
        ...
        Alguns sinais não obedecem essa classificação, por exemplo o sinal $g(t) = e^{-at}$
...
A transformada de Fourier de um sinal determinístico $x(t)$ fornece uma representação no domínio da frequência $X(\omega) = |X(\omega)|e^{j\theta(\omega)}$ na qual é possível analisar o módulo e a fase, sendo esse último parâmetro importante para a análise da distorção durante a transmissão dos sinais.

Entretanto, para sinais aleatórios a aplicação da transformada de Fourier leva a uma representação que também é aleatório, uma para cada realização do sinal. Para que possamos analisar a densidade espectral de potência de um sinal aleatório precisamos de uma medida que nos forneça o comportamento médio do sinal, e essa medida é a autocorrelação, como será mostrado nessa seção. 
...
\end{equation}
tomemos novamente $g_T(t)$ e sua densidade espectral de energia $\mid G_T(\omega)\mid^2$, então
\begin{equation}
...
Deve ser lembrado que a DEP contém somente informação de magnitude do sinal, pois a informação de fase não é utilizada. Assim, para cada sinal só há uma DEP, porém para cada DEP $S_x(\omega)$ pode haver uma infinidade de sinais no tempo, que diferem entre si pela fase.
...
\subsection{Interpretação da DEP}

Também similar a interpretação feita para a densidade espectral de
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.5\textwidth]{Figuras/pulsosbinariosaleatorios.png}
\end{center}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.5\textwidth]{Figuras/pulsosbinariosaleatoriosdeslocados.png}
\end{center}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.5\textwidth]{Figuras/autocorrelacaopulsosbinarios.png}
\end{center}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.5\textwidth]{Figuras/psdpulsotriangular.png}
\end{center}
...
\begin{exercicio} Seja um sinal $x(t) = \cos\omega_1 t$ transmitido através de um canal multipercurso (dois raios) em que $\omega_1 t_d = \pi$, $\omega_1(t_d+\Delta T) = 2\pi$ e $\alpha = 1$. Qual o sinal recebido?
\end{exercicio}